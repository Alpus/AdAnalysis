...
def usecase3():
    step=0.01
    u_func = OneArgFunction(str_func='3 * (y ^ 2) - 2 * (y ^ 3)', arg='y', begin=0, end=1, step=step)
    s_func = OneArgFunction(str_func='3 * t + sin(t)', arg='t', begin=0, end=1, step=step)
    z_func = OneArgFunction(str_func='4 * t + cos(t)', arg='t', begin=0, end=1, step=step)
    z1_func = OneArgFunction(str_func='4 - sin(t)', arg='t', begin=0, end=1, step=step)
    print(
        solve_cauchy(
